-- This migration fixes a bug in the get_calendar_appointments RPC function
-- where the time range filtering was incorrect, causing no appointments to be returned.
-- The WHERE clause is updated to use the correct overlap operator (&&) for time ranges.
...
BEGIN
    ...
    WHERE
        -- FIX: Use the tstzrange overlap operator '&&' for correct date filtering.
